    ...
    if ($_POST['action'] == 'update_profile') {
        try {
            ...
            if (!empty($_POST['password'])) {
                if (getUserRole() == 'mahasiswa') {
                    $pass_query = "UPDATE mahasiswa SET password = :password WHERE id_mahasiswa = :user_id";
                } elseif (getUserRole() == 'dosen') {
                    $pass_query = "UPDATE dosen SET password = :password WHERE id_dosen = :user_id";
                } else {
                    $pass_query = "UPDATE admin SET password = :password WHERE id_admin = :user_id";
                }
            ...
            if (getUserRole() == 'mahasiswa') {
                $profile_query = "UPDATE mahasiswa SET 
                                 nama = :nama_lengkap, tanggal_lahir = :tanggal_lahir, 
                                 jenis_kelamin = :jenis_kelamin, alamat = :alamat, 
                                 nomor_telepon = :no_telepon, email = :email
                                 WHERE id_mahasiswa = :user_id";
            } elseif (getUserRole() == 'dosen') {
                $profile_query = "UPDATE dosen SET 
                                 nama_dosen = :nama_lengkap, email = :email,
            ...
            if (isset($profile_query)) {
                ...
                $profile_stmt->bindParam(':email', $_POST['email']);
                            ...
                            <div class="space-y-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="tanggal_lahir"
                                        value="<?php echo $profile['tanggal_lahir'] ?? ''; ?>"
                                        class="form-input w-full">
                                </div>
                                ...
                                <div>
                                    ...
                                    <select name="jenis_kelamin" class="form-input w-full">
                                        ...
                                        <option value="L"
                                            <?php echo ($profile['jenis_kelamin'] ?? '') == 'L' ? 'selected' : ''; ?>>
                                            Laki-laki</option>
                                        <option value="P"
                                            <?php echo ($profile['jenis_kelamin'] ?? '') == 'P' ? 'selected' : ''; ?>>
                                            Perempuan</option>
                                    </select>
                                </div>
                                ...
                                <div>
                                    ...
                                    <textarea name="alamat" rows="4"
                                        class="form-input w-full"><?php echo htmlspecialchars($profile['alamat'] ?? ''); ?></textarea>
                                </div>
                                <?php endif; ?>
                                ...
                                <div>
                                    ...
                                    <input type="tel" name="no_telepon"
                                        value="<?php echo htmlspecialchars($profile['nomor_telepon'] ?? ''); ?>"
                                        class="form-input w-full">
                                </div>
                            </div>
                        </div>
